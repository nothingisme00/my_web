...
type ColorVariant = 'blue' | 'purple' | 'green' | 'orange' | 'teal' | 'rose' | 'indigo';
...
const colorClasses: Record<ColorVariant, { bg: string; text: string; badge: string }> = {
  blue: {
  ...
  teal: {
    ...
    badge: 'bg-teal-100 dark:bg-teal-900/30 text-teal-700 dark:text-teal-300 border-teal-200 dark:border-teal-800',
  },
  rose: {
  ...
  indigo: {
    ...
    badge: 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 border-indigo-200 dark:border-indigo-800',
  },
};
      ...
      <div className="flex-1">
        <div className="flex items-center gap-2">
          ...
          {itemCount !== undefined && itemCount > 0 && (
            <span className={clsx('px-2 py-0.5 text-xs font-medium rounded-full border', colors.badge)}>
              {itemCount}
            </span>
          )}
        </div>
